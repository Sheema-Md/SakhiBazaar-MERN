...
const MarketPriceWidget = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [pricesRes, trendsRes] = await Promise.all([
        axios.get(`${MAIN_API_URL}/market/market-prices`),
        axios.get(`${MAIN_API_URL}/market/market-trends`)
      ]);
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-slate-800 border border-rose-100/30 dark:border-slate-700/60 p-6 rounded-3xl shadow-sm flex flex-col items-center justify-center min-h-[220px]">
  ...
  if (error) {
    return (
      <div className="bg-white dark:bg-slate-800 border border-rose-100/30 dark:border-slate-700/60 p-6 rounded-3xl shadow-sm text-center text-xs text-slate-400">
  ...
  return (
    <div className="bg-white dark:bg-slate-800 border border-rose-100/30 dark:border-slate-700/60 rounded-3xl shadow-md p-5 space-y-4">
      {/* Header Info */}
        ...
        <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1">
          {prices.map((item, idx) => {
            ...
            return (
              <div
                key={idx}
                className="flex items-center justify-between p-3 bg-slate-50/50 dark:bg-slate-900/10 border border-rose-100/20 dark:border-slate-750/50 rounded-2xl hover:border-slate-250 dark:hover:border-slate-700 hover:bg-slate-55 dark:hover:bg-slate-900/30 transition-all"
              >
                ...
                <div className="min-w-0 flex-1 pr-3">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {selectedSeason !== 'All' && seasonalDiffPercent !== 0 && (
                      <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-md uppercase leading-none ${seasonalDiffPercent > 0
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-1">
                  {item.popularProducts?.map((prod, pIdx) => (
                    <span
                      key={pIdx}
                      className="px-2 py-0.5 bg-white dark:bg-slate-800 border border-rose-100/10 dark:border-slate-700 text-slate-650 dark:text-slate-350 rounded-lg text-[9px] font-bold flex items-center gap-0.5"
                    >
